...
model = Model.load_from_file('example_model.json', lang_classes_factory)

graph = AttackGraph(lang_graph, model)
graph.attach_attackers()
os_app_local_access = graph.get_node_by_id('OS App:localAccess')
os_app_np = graph.get_node_by_id('OS App:notPresent')
os_app_np.defense_status = 1.0

graph.save_to_file('ag.json')

apriori.calculate_viability_and_necessity(graph)

graph.save_to_file('post_ag.json')

attacker = graph.attackers[0]
attacker.compromise(graph.get_node_by_id('OS App:networkConnect'))
attacker.compromise(graph.get_node_by_id('OS App:authenticate'))
...
for step in query.get_attack_surface(graph, graph.attackers[0]):
    print(step.id)
